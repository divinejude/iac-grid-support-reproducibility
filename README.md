...
- Optional ANDES IEEE 14-bus nonlinear multi-machine transient-stability validation
- Prototype ANDES/OpenDSS T&D dynamic co-simulation with 100 ms explicit coupling
- Inverter current-loop bandwidth, current saturation, DC-link limit, and anti-windup
- Averaged inverter validation with SRF-PLL, dq PI current control, L-filter dynamics, PWM delay, and DC-link capacitance
...
- Decentralized MPC active-power dispatch with strict thermal comfort constraints
- Network-aware full P/Q DMPC using OpenDSS finite-difference voltage sensitivity
...
Run the prototype iterative T&D dynamic co-simulation:
...
This consumes the latest full P/Q DMPC source-sag trajectory, simulates an averaged single-phase inverter over the stressed transition, injects an additional fast voltage notch and frequency pulse, and writes:
